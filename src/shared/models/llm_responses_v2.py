...
class HallucinationChecklist(BaseModel):
    decimal_rates_correct: Literal["pass", "fail"] = Field(
        description="Are all rate fields (CTR, CVR) correctly interpreted as decimals, e.g. 0.035 = 3.5%?"
    )
    derived_metrics_consistent: Literal["pass", "fail"] = Field(
        description="Are derived metrics (CPA, ROAS, CPM) consistent with the raw input values?"
    )
    no_external_benchmarks: Literal["pass", "fail"] = Field(
        description="Does the report avoid referencing benchmarks or industry standards not present in the data?"
    )
    no_trend_claims_without_timeseries: Literal["pass", "fail"] = Field(
        description="Does the report avoid trend claims (e.g. 'improving', 'declining') without time-series data?"
    )
    no_unsupported_audience_claims: Literal["pass", "fail"] = Field(
        description="Does the report avoid claims about audience behavior not supported by the data?"
    )
    no_values_stated_for_null_fields: Literal["pass", "fail"] = Field(
        description="Does the report avoid stating values for fields that are null in the raw input?"
    )
