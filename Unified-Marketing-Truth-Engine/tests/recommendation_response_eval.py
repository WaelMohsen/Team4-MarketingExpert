...
class EvaluateRecommendation(dspy.Signature):
    ...
    clarity_reasoning = dspy.OutputField(
        desc=(
            "Explain whether the recommendation is clear, specific, and actionable. " # TODO: add more details
            "Mention if steps, metrics, or examples are missing or vague."
        )
    )
    clarity_score = dspy.OutputField(
        desc=(
            "Integer score (1-5): "  # TODO:
    ...
    accuracy_reasoning = dspy.OutputField(
        desc=(
            "Explain whether the recommendation logically follows from the analysis_context. "  # TODO: add details for best practices from marketing domain
    ...
    accuracy_score = dspy.OutputField(
        desc=(
            "Integer score (1-5): "
            "1=incorrect/misleading, 3=partially correct, 5=fully grounded and logically sound." # TODO: add more details
        )
    )

    # Non-redundancy
    redundancy_reasoning = dspy.OutputField(     # TODO: rewrite variable name
        desc=(
...
if __name__ == "__main__":
    ...
    if args.files:
        for file_path in args.files:
            ...
            output_name = f"eval_{file_path_obj.stem}_{timestamp}.json"
